...
Меню выполнено через template tag, а именно в таких местах как наследование от base.html {% extends "menu/base.html" %} а так же блоков contenta {% block content %}
...
<h2>3) Хранится все данные должны в БД.</h2>
...
<h2>4) Реализовать возможность редактировать все данные в стандартной админке Django</h2>
В вход в django admin был реализован через главную кнопку
...
После создания и миграции моделей они были зарегистрированы в приложение admin для взаимодействия с ними в панели admin
...
Переходы по ссылками реализованы в urls.py
...
<h2>8)На отрисовку каждого меню требуется ровно 1 запрос к БД</h2>
...
Формирует 2 запроса SQL касаемые пользователей
а, страница меню формирует 6 запросов 2 из 6 запросов к пользователю и 3 к элеменатам выводящим объекты БД 
